...
export default function Wizard({ formData, onChange, activeTab, onNext, onPrev, validationResults, extractedData }) {
  ...
  /* ── Read-only preview of a boilerplate paragraph the generator writes automatically
     (no field to fill — shown so the user knows exactly what lands in the docx). ── */
  const AutoGeneratedPreview = ({ title, text }) => (
    <div className="mb-4 rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-3.5 py-2 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
        ...
        <span className="text-[9px] font-bold uppercase tracking-wide text-gray-400 bg-white border border-gray-200 px-1.5 py-0.5 rounded-md">Auto-generated · read only</span>
      </div>
      <p className="px-3.5 py-3 text-[11.5px] text-gray-500 leading-relaxed italic">{text}</p>
    </div>
  );
        ...
        <div className="px-7 py-6">
          ...
          {activeTab === 'cover' && (
            <div>
              ...
              <SubGroupHeader icon={AlertTriangle} label="Auto-Generated Boilerplate" note="No input needed — previewed here for reference" />
              <AutoGeneratedPreview title="Risks in Relation to the First Offer" text={`This being the first public issue of Equity Shares of our Company, there has been no formal market for the Equity Shares. The face value of each Equity Share is ₹${formData.face_value_per_share || '[●]'}. The Offer Price, Floor Price and the Cap Price will be determined in consultation with the BRLM…`} />
              <AutoGeneratedPreview title="General Risk / Company's Absolute Responsibility / Listing" text="Standard SEBI ICDR boilerplate on investment risk, issuer/selling-shareholder responsibility, and stock exchange listing (BSE/NSE) — generated automatically from fixed regulatory language plus the fields above." />
